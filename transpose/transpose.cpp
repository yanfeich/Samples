...
int main(int argc, char *argv[])
{
    ...
    std::vector<int32_t> permutation_data = {0, 2, 1};
    auto rank = permutation_data.size();
    for (size_t i = 0; i < permutation_data.size(); i++)
    {
        permutation_data[i] = rank - 1 - permutation_data[i];
    }
    std::reverse(permutation_data.begin(), permutation_data.end());
    for (size_t i = 0; i < permutation_data.size(); i++)
    {
        params.permutation[i] = static_cast<TransposePermutationDim>(permutation_data[i]);
    }

    params.tensorDim = permutation_data.size();
    ...
    std::string name("transpose.recipe");
