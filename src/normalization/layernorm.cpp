...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 0; i < dtypes.size(); i++)
    {
        ...
        unsigned X_shape[] = {H, T, B, N};
        unsigned X_size = H * T * B * N;

        unsigned Y_shape[] = {H, T, B, N};
        unsigned Y_size = H * T * B * N;
        ...
        unsigned beta_shape[] = {H};
        unsigned beta_size = H * 4;
        synSectionHandle beta_SectionHandle = nullptr;
        status = synSectionCreate(&beta_SectionHandle, 0, graphHandle);
        assert(status == synSuccess && "Failed to call synSectionCreate()");

        synTensorDescriptor beta_desc;
        beta_desc.m_dataType = syn_type_single;
        beta_desc.m_dims = 1UL;
        beta_desc.m_name = "Beta";
        memset(beta_desc.m_strides, 0, sizeof(beta_desc.m_strides));
        memset(beta_desc.m_sizes, 0, sizeof(beta_desc.m_sizes));
        memcpy(beta_desc.m_sizes, beta_shape, 1 * sizeof(unsigned));
        synTensor syn_beta = nullptr;
        status = synTensorCreate(&syn_beta, &beta_desc, beta_SectionHandle, offset);
        assert(status == synSuccess && "Failed to call synTensorCreate()");
        inputs.push_back(syn_beta);
        ...
        unsigned mean_square_shape[] = {1, T, B, N};
        unsigned mean_square_size = B * T * N * 4;
        ...
        memcpy(mean_square.m_sizes, mean_square_shape, 4 * sizeof(unsigned));
        ...
        //************************************************ create gamma grad tensor
        unsigned std_shape[] = {1, T, B, N};
        unsigned std_size = B * T * N * 4;
        synSectionHandle std_SectionHandle = nullptr;
        status = synSectionCreate(&std_SectionHandle, 0, graphHandle);
        assert(status == synSuccess && "Failed to call synSectionCreate()");

        synTensorDescriptor std_desc;
        std_desc.m_dataType = syn_type_single;
        std_desc.m_dims = 4UL;
        std_desc.m_name = "std";
        memset(std_desc.m_strides, 0, sizeof(Y_desc.m_strides));
        memset(std_desc.m_sizes, 0, sizeof(Y_desc.m_sizes));
        memcpy(std_desc.m_sizes, std_shape, 4 * sizeof(unsigned));
        synTensor syn_std = nullptr;

        status = synTensorCreate(&syn_std, &std_desc, std_SectionHandle, offset);
        assert(status == synSuccess && "Failed to call synTensorCreate()");
        outputs.push_back(syn_std);

        //************************************************ create rms node
        std::string guid = "layer_norm_fwd_" + dtypes[i];
        std::string node_name = "LayerNorm";
        ns_LayerNormKernel::Params params;
        params.epsValid = true;
        params.eps = float(1e-5);
