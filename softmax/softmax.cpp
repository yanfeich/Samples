...
int main(int argc, char *argv[])
{
    ...
    //************************************************ create rms node
    std::vector<std::string> unary_guids = {"relu", "abs", "exp", "sigmoid", "sign", "sqrt", "tanh", "erf"."log", "neg", "floor", "ceil", "cos", "acos", "sin", "asin", "sinh", "tan", "softsign"};
    ...
    for (size_t i = 0; i < unary_guids.size(); i++)
    {
        for (size_t j = 0; j < dtypes.size(); j++)
        {
            ...
            std::string guid = "softmax_fwd_bf16";
            std::string node_name = guid + "_" + std::string(i) + "_" + std::string(j);
